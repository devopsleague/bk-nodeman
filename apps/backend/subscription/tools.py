...
def get_service_instances(bk_biz_id: int, bk_host_list: List[int]) -> List[Dict]:
    ...
    try:
        ...
        while True:
            ...
            batch_result = CCApi.list_service_instance_detail(params)
            result.extend(batch_result["info"])
            if batch_result["count"] != constants.QUERY_CMDB_LIMIT:
                break

            params["page"]["limit"] += constants.QUERY_CMDB_LIMIT
    except Exception:
